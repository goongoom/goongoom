...
export async function getClerkUserByUsername(username: string): Promise<ClerkUserInfo | null> {
  try {
...
export async function getClerkUserById(clerkId: string): Promise<ClerkUserInfo | null> {
  try {
    const client = await clerkClient()
